...
class MainWindow(QMainWindow):
    ...
    def run_drone(self):
        ...
        #        print(" Không tìm thấy thông tin vị trí đã chọn")
            ...
            mode_1 = False
            self.uic.textEdit_location.setEnabled(False)      
            self.send_goal(0.4,0,0.707,0.707)
            z = 0.8
...
class connect_drone(QThread):
    ...
    def run(self):
        ...
        global hang, ke, check, cot
        global goal_y, enable_x_2
        capture = False
        enable_x_1 = False
        enable_x_2 = True
        pid = [1, 1, 0]
                ...
                if Done:
                    if mode_2:  #mode auto
                        ...
                        if (error_z < -0.1) or (error_z > 0.1):
                            action[0][2] = pid[0]*error_z*2 + pid[1]*(error_z - pre_error_z)
                        else:
                            action[0][2] = 0
                            enable_x_1 = True

                        if enable_x_1 and enable_x_2 :
                            error_x = goal_x - x
                            if (error_x < -0.1) or (error_x > 0.1):
                                ...
                                if cot > 8 or cot <1:
                                    hang = hang + (-1)**(ke+1)
                                    if hang > 6 or hang < 1:  # chỉnh số lượng hàng
                                        hang = hang + (-1)**(ke)
                                        Done = False
                                        capture = False
    ...
    def img_cb(self, data):
        global capture, location, lan,t, enable_x_2
        global hang, ke,cot, x,y,goal_y, check, mode_1, mode_2
        ...
        #             self.check_qrcode_mode_2(self.qr_codes_scanned_2,self.location_qrcode)
        #             self.qr_codes_scanned_2 = []  
        #             self.location_qrcode = []
        #         else:
        #             print("Không quét được QR Code nào")   
    ...
    def check_qrcode_mode_2(self,qr_code,location):
        ...
        #     self.message_signal.emit(f"Thiếu QR_Code: {qr_codes_str}")
        # self.message_signal.emit(f"Finished!")
